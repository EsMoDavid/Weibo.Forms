...
if(bases.length == 0){
    head.innerHTML = 'baseTag' + head.innerHTML;
        ...
        void OnNavigationCompleted(Windows.UI.Xaml.Controls.WebView sender, WebViewNavigationCompletedEventArgs args)
        {
            ...
            Element.HandleNavigationCompleted(args.Uri==null?string.Empty:args.Uri.ToString());
        }
        ...
        void SetSource()
        {
            ...
            switch (Element.ContentType)
            {
                ...
                case WebViewContentType.StringData:
                    LoadStringData(Element.Source,Element.BaseUrl);
                    break;
        ...
        void LoadStringData(string source, string baseUrl)
        {
            if (!string.IsNullOrEmpty(baseUrl))
            {
                ...
                TypedEventHandler<Windows.UI.Xaml.Controls.WebView, WebViewNavigationCompletedEventArgs> handler = null;
                handler = async (sender, args) =>
                {
                    Control.NavigationCompleted -= handler;
                    ...
                    Control.NavigateToString(!string.IsNullOrEmpty(htmlWithBaseTag) ? htmlWithBaseTag : source);
                };
